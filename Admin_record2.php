...
<div class="container">
	...
	<ul>
        <li><button class="nav-btn" onclick="javascript:btnStudentS()">Student Borrow Book Statistic(Summary)</button ></li>
        <li><button class="nav-btn" onclick="javascript:btnStudentD()">Student Borrow Book Statistic(Details)</button ></li>
        <li><button class="nav-btn" onclick="javascript:btnBook()">Most Viewed Book of the Month</button></li>
    </ul>
    </div>
	<div  id="container-S" style="display:none">
	...
	<div id="container-D">
		<form action="Admin_record2.php" method="post">
        	<div class="search-container" >
        		<select name="category-D" class="search" style="width:100px">
        			<option value="Bookname">Book Name</option>
        			<option value="Username">Username</option>
        		</select>
				<input type="text" class="search" name="search_key-D" placeholder="Key in book name to search...">
				<span>
        			<button type="submit" class="submit">Search</button>
				</span>
			</div>
		...
		<?php
			$search = isset($_POST['search_key-D']) ? $_POST['search_key-D'] : '';
			$category = isset($_POST['category-D']) ? $_POST['category-D'] : '';
			...
			{	
				if ($search == NULL)
				{
		  			$result = mysqli_query($con,"SELECT record.StudentID, student.Username, record.RmID, reading_material.Rm_name, COUNT(record.RmID) FROM record join reading_material on record.RmID = reading_material.RmID join student on record.StudentId = student.StudentID group by record.StudentID, record.RmID order by record.RmID ASC");
		  		}
		  		else
		  		{
		  			$result = mysqli_query($con,"SELECT record.StudentID, student.Username, record.RmID, reading_material.Rm_name, COUNT(record.RmID) FROM record join reading_material on record.RmID = reading_material.RmID join student on record.StudentId = student.StudentID WHERE reading_material.Rm_name LIKE '%" . $search . "%' group by record.StudentID, record.RmID order by record.RmID ASC");
		  		}
		  	}
		  	elseif ($category == 'Username')
		  	{
				if ($search == NULL)
				{
		  			$result = mysqli_query($con,"SELECT record.StudentID, student.Username, record.RmID, reading_material.Rm_name, COUNT(record.RmID) FROM record join reading_material on record.RmID = reading_material.RmID join student on record.StudentId = student.StudentID group by record.StudentID, record.RmID order by record.StudentID ASC");
		  		}
		  		else
		  		{
		  			$result = mysqli_query($con,"SELECT record.StudentID, student.Username, record.RmID, reading_material.Rm_name, COUNT(record.RmID) FROM record join reading_material on record.RmID = reading_material.RmID join student on record.StudentId = student.StudentID WHERE student.Username LIKE '%" . $search . "%' group by record.StudentID, record.RmID order by record.StudentID ASC");
		  		}
		  	}
		  	else
		  	{
