...
$result = $mysqli->query
("select * from reading_material join book_gen on reading_material.RmID = book_gen.RmID where reading_material.RmID = $bid group by reading_material.RmID")
or die($mysqli->error);
...
while ($reading_material = $result->fetch_assoc()):?>
 ...
 <div class='movie-container'>
    ...
    <?php
    $resultx = $mysqli->query
("select * from rating where StudentID = $id and RmID = $bid")
or die($mysqli->error);
...
if ($resultx->num_rows > 0) {
    ...
    while($row1 = $resultx->fetch_assoc()) {
        ...
        echo "<label style='position:absolute;left:15%;top:66.5%;'>My Rating:</label></br>";
        echo"
        <div class='rating' >
            <input type='radio' name='star' id ='star1'  value='5'";if ($row1['Score']==5){echo "checked='checked'";}else{} echo"><label for='star1'>
            </label>
            <input type='radio' name='star' id ='star2' value='4'";if ($row1['Score']==4){echo "checked='checked'";}else{} echo"><label for='star2'>
            </label>
            <input type='radio' name='star' id ='star3' value='3'";if ($row1['Score']==3){echo "checked='checked'";}else{} echo"><label for='star3'>
            </label>
            <input type='radio' name='star' id ='star4' value='2'";if ($row1['Score']==2){echo "checked='checked'";}else{} echo"><label for='star4'>
            </label>
            <input type='radio' name='star' id ='star5' value='1'";if ($row1['Score']==1){echo "checked='checked'";}else{} echo"><label for='star5'>
            </label>
        
        </div>";
        echo "<input type='hidden' type='int'  name='rid' value='$rid'></input>";
        echo "<label style='position:absolute;left:15%;top:69%;'>My Comment:</label>";
        echo "<td><textarea style='position:absolute;left:22%; width:800px; top:69.5%;'name='content' id='content1' rows='5'>";
         echo $row1['Comment'];
         echo "</textarea></td>";
         echo "<button style='background:Grey;padding:5px 12px;position:absolute;left:68%;top:82%;'type='submit'>Update</button>";
         echo "</form>";
         echo "</div>";
    ...
    echo "<div class='comment-container'>";
    echo "<form action='insertr.php' method='post'>";
    echo "<label style='position:absolute;left:15%;top:66.5%;'>My Rating:</label></br>";
    echo"
    <div class='rating'>
        ...
        <input type='radio' name='star' id ='star5' id='c' value='1'><label for='star5'>
        </label>
    
    </div>";
    echo "<input type='hidden'type='int'  name='id' value='$bid'></input>";
    echo "<input type='hidden'type='int'  name='sid' value='$id'></input>";
    echo "<label style='position:absolute;left:15%;top:69%;'>My Comment:</label>";
    echo "<td><textarea style='position:absolute;left:22%; width:800px; top:69.5%;'name='content' id='content1' rows='5'>";
     echo "</textarea></td>";
     echo "<button style='background:Grey;padding:5px 12px;position:absolute;left:68%;top:82%; onclick='check()' 'type='submit'>Insert</button>";
     echo "</form>";
     echo "</div>";
...
?>
    ...
    <?php
    $resultxx = $mysqli->query
    ("select rating.Score, rating.Comment, student.Username from rating join student on rating.StudentID = student.StudentID where rating.StudentID <> $id and rating.RmID = $bid")
    or die($mysqli->error);?>


    <?php while ($rst = $resultxx->fetch_assoc()):?>
    <div class='allcomment-container'>
        <span style='position:absolute;left:15%;background-color:rgb(211, 238, 211);'>Username:</span>
        <span style='position:absolute;left:20%;background-color:rgb(211, 238, 211);'><?= $rst['Username'] ?></span>

        <span style='position:absolute;left:30%;background-color:rgb(211, 238, 211);'>Rating:</span>
        
        <span style='position:absolute;left:33.5%;background-color:rgb(211, 238, 211);'><?php 
        $xx = $rst['Score']; 
        if ($xx == 1){
            echo "<img src='image/star1.png' width='100' height='20'>";
        }else if($xx == "2"){
            echo "<img src='image/star2.png' width='100' height='20'>";
        }else if($xx =="3"){
            echo "<img src='image/star3.png' width='100' height='20'>";
        }else if($xx =="4"){
            echo "<img src='image/star4.png' width='100' height='20'>";
        }else{
            echo "<img src='image/star5.png' width='100' height='20'>";
        }
        ?>
        
        </span>
         
        <span style= "Display:block; height: 25px; background-color:rgb(211, 238, 211)"></span>
        
        <span style='position:absolute;left:15%;background-color:rgb(211, 238, 211);'>Comment:</span>
        <td><textarea style="position:absolute;left:22%; width:800px;background:rgba(235, 232, 232, 0.808);" name='content' id='content1' rows='5' readonly><?= $rst['Comment']?></textarea></td>
